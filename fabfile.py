...
def symlink_nginx():
    ...
    with cd(env.path):
        sudo('ln -sf $PWD/server/nginx/{host}.conf '
            '{nginx_conf_dir}/harvest-site-{host}.conf'.format(**env))
...
def reload_supervisor():
    ...
    with cd(env.path):
        sudo('ln -sf $PWD/server/supervisor/{host}.ini '
            '{supervisor_conf_dir}/harvest-site-{host}.ini'.format(**env))
...
def upload_settings():
    ...
    if os.path.exists(local_path):
        remote_path = os.path.join(env.path, 'src/conf/local_settings.py')
